...
struct AppLifecycleTests {
    @Test func regularAppHasFastNavigationReopenAndClearExitPaths() throws {
        ...
        #expect(settingsSource.contains("保存自定义语音动作"))
        #expect(settingsSource.contains("保存后会像 Codex、微信一样出现在语音键动作列表中"))
        #expect(settingsSource.contains("settings.customVoiceActions"))
