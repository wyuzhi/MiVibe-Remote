...
本文适用于 MiVibe Remote 0.1.7。请先确认 Mac 为 Apple Silicon，系统版本为 macOS 26 或更高。
...
默认开启“遥控器语音时临时切换系统麦克风”：空闲时应用不会修改系统输入；
按住遥控器语音键时切到 `MiRemoteV 2ch`，松开并完成尾音传输后恢复原来的
耳机、电脑或其他麦克风。应用不会修改系统默认输出。
如果目标应用固定选择了某个麦克风而不是“系统默认”，仍需在目标应用中选择
`MiRemoteV 2ch`。

## 连接蓝牙耳机后几乎识别不到语音

1. 在“连接 → 高级设置”确认“遥控器语音时临时切换系统麦克风”已经开启；
2. “虚拟麦克风”状态应显示“已就绪”，设备切换期间会显示“自动恢复中”；
3. 按住遥控器语音键期间系统输入应为 `MiRemoteV 2ch`，松开后应恢复此前的输入设备；
...
```bash
shasum -a 256 -c MiVibe-Remote-0.1.7.dmg.sha256
```
