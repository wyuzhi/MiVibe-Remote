...
本文适用于 MiVibe Remote 0.1.14。请先确认 Mac 为 Apple Silicon，系统版本为 macOS 15 或更高。
...
`MiRemoteV 2ch`，但空闲时音频通道处于关闭状态。按住语音键后才启动并传输
遥控器声音，尾音发送完成后再次关闭。按键过程不修改系统输入设备，应用也不会
修改系统默认输出。只有主动开启“电脑麦克风透传”时才会持续采集 MacBook 麦克风。
如果目标应用固定选择了某个麦克风而不是“系统默认”，仍需在目标应用中选择
`MiRemoteV 2ch`。

## 连接蓝牙耳机后几乎识别不到语音

1. 在“连接 → 高级设置”确认“保持虚拟麦克风为默认输入”已经开启；
2. “虚拟麦克风”状态应显示“已就绪”；默认情况下电脑麦克风透传应保持关闭；
3. 无论是否按住语音键，系统默认输入都应保持为 `MiRemoteV 2ch`；
4. 如果状态一直停在“自动恢复中”，点击“重新选择虚拟麦克风”后重试；
5. 对于在应用内部固定麦克风的目标应用，请把其麦克风改为 `MiRemoteV 2ch`。

耳机连接和断开可能让 macOS 重建 CoreAudio 路由。MiVibe 会重新绑定虚拟输出并
把默认输入重新设为 `MiRemoteV 2ch`，但不会在空闲时开启采集通道。主动开启
电脑麦克风透传后，MiVibe 才会明确采集 MacBook 内置麦克风，不会跟随蓝牙
耳机切换到低质量通话输入。
...
```bash
shasum -a 256 -c MiVibe-Remote-0.1.14.dmg.sha256
```
