...
本文适用于 MiVibe Remote 0.1.10。请先确认 Mac 为 Apple Silicon，系统版本为 macOS 26 或更高。
...
空闲时传入 MacBook 内置麦克风，按住语音键时由遥控器接管，松开后恢复电脑
麦克风。按键过程不修改系统输入设备，应用也不会修改系统默认输出。
如果目标应用固定选择了某个麦克风而不是“系统默认”，仍需在目标应用中选择
`MiRemoteV 2ch`。

## 连接蓝牙耳机后几乎识别不到语音

1. 在“连接 → 高级设置”确认“常驻虚拟麦克风”已经开启；
2. “虚拟麦克风”状态应显示“已就绪”，电脑语音状态应显示 MacBook 麦克风已连接到 `MiRemoteV 2ch`；
...
把默认输入重新设为 `MiRemoteV 2ch`，但电脑语音始终明确采集 MacBook 内置麦克风，
不会跟随蓝牙耳机切换到低质量通话输入。
...
```bash
shasum -a 256 -c MiVibe-Remote-0.1.10.dmg.sha256
```
