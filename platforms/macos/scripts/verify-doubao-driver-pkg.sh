...
case "$MODE" in
  install)
    ...
    /usr/bin/grep -Fqx "/usr/bin/vtool -show-build \"\$BINARY\" | /usr/bin/grep -q 'minos 15\\.0'" "$EXPANDED/Scripts/postinstall"
    /usr/bin/grep -Fqx "/usr/bin/vtool -show-build \"\$APP_BINARY\" | /usr/bin/grep -q 'minos 15\\.0'" "$EXPANDED/Scripts/postinstall"
