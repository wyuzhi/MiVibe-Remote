...
正式发布包每天自动检查一次更新，也可以从应用菜单或右键菜单栏图标后选择“检查更新…”。发现新版后必须由用户确认，应用不会在后台静默下载安装。Sparkle 仅更新应用本体，兼容麦克风驱动仍由 DMG 中的安装包管理。未注入发布更新源的开发包不会访问网络，并会在菜单中显示“自动更新未配置”。
...
每个普通按键都可以设置单击动作，并可额外设置双击和长按动作。动作支持键盘操作、鼠标滚轮、系统音量、播放控制、打开当前 Mac 已安装的常用应用，以及录入任意自定义键盘快捷键。
