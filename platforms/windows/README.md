...
.\delivery\build-standalone-packages.ps1 -Version 0.1.13 -Product xiaomi -AllowUnsignedCandidate
```

默认只构建 MiVibe Remote；生成的可执行文件和安装器分别为
`MiVibeRemote.exe` 与 `MiVibeRemoteSetup-0.1.13.exe`。`-Product` 参数仍可显式选择
