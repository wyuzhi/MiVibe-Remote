...
class ValidateUpdatePromotionTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.metadata = expected_assets("v0.1.16", self.plist)

    def tearDown(self) -> None:
        self.temp.cleanup()

    def test_expected_asset_names_are_bound_to_tag_and_source_plist(self) -> None:
        self.assertEqual(self.metadata["windowsVersion"], "0.1.16")
        self.assertEqual(
            self.metadata["windowsFilename"], "MiVibeRemoteSetup-0.1.16.exe"
        )
    ...
    def test_non_stable_or_malformed_tag_is_rejected(self) -> None:
        for tag in ("0.1.16", "v0.1", "v0.1.16-beta.1", "update-feed"):
            with self.subTest(tag=tag), self.assertRaisesRegex(ValueError, "tag"):
                expected_assets(tag, self.plist)
    ...
    def test_both_platform_versions_must_strictly_increase(self) -> None:
        paths = {
        ...
        for platform, version in (("old_mac", "11"), ("old_windows", "0.1.16")):
            with self.subTest(equal_platform=platform):
                write_appcast(paths[platform], version)
                ...
                write_appcast(
                    paths[platform], "10" if platform == "old_mac" else "0.1.15"
                )
