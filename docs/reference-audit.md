...
- `HD838A/remote-mic-app` 已复查到公开主线提交 `1d3f28f`。本轮重点审计了它新增的
  HID 权限恢复、被系统占用快捷键的事件捕获、首次使用端到端门禁、输入法会话恢复和
  小窗口响应式布局。
- MiVibe 本轮吸收其中三个与当前用户问题直接相关的原则：权限变化后自动重建 HID、
  应用内提供可操作的使用教程与重试入口、设置页在小窗口下使用响应式/可滚动布局。
- `VincentKingHsu/MiRemoteVoice` 在本次复查时已无法通过原公开地址获取；保留此前已完成的
  `v1.0.0-beta.2` 协议审计结论，不把无法复核的新变化写成事实。
- 没有复制上游品牌、图片或整页实现；新增功能继续落在 MiVibe 自己的数据模型、界面和测试中。
